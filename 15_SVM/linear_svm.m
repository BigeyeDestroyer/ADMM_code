...
% A(j, :) = [-y_j * x_j', -y_j]
% 
% lambda is the penalty coefficient for the outliers
%
% p is a partition of the observations into different subsystems.
%
...
%% ADMM solver 
x = zeros(n,N); % N, number of partitions 
z = zeros(n,N); % each column a parameter for corresponding cluster 
u = zeros(n,N);
...
for k = 1:MAX_ITER
    ...
    for i = 1:N
        cvx_begin quiet
            variable x_var(n)
            minimize sum(pos(1 + A{i} * x_var)) + (rho / 2) * sum_square(x_var - z(:, i) + u(:, i))
        cvx_end
        x(:, i) = x_var;
    end
    xave = mean(x,2);
...
function obj = objective(A, lambda, ~, x, z)
